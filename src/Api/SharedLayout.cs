...
public static class SharedLayout
{
    ...
    private static string Resolve(string b,string lang,Snapshot s,List<string> issues)
    {
        ...
        var value=b switch
        {
            ...
            "PROIONTA.PROION"=>product?.Names.GetValueOrDefault(lang)??"",
            ...
            "PROIONTA.NOPO_KTPS"=>s.Languages.GetValueOrDefault(lang)?.Headings.GetValueOrDefault(product?.Frozen==true?"frozen":"fresh")??"",
            "NOPO_KTPS.KATASTASH_EN"=>s.Languages.GetValueOrDefault(lang)?.Headings.GetValueOrDefault(product?.Frozen==true?"frozen":"fresh")??"",
