...
public static class Validation
{
    ...
    public static string Check(string kind,JsonElement data)
    {
        ...
        switch(kind)
        {
            case "product":var p=Json.Read<Product>(raw);if(!Rules.ErpPattern().IsMatch(p.ErpCode)||p.ShelfLife is <0 or >9999||p.Names.Count==0)throw new InvalidOperationException("Ελέγξτε κωδικό ERP, ημέρες λήξης και περιγραφές.");break;
